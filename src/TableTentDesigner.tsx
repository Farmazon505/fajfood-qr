...
import type { DiningTable, VenueSettings } from '../server/types';
...
export function TableTentDesigner({ tables, settings, publicUrl }: Props) {
  ...
  }, [callToAction, footerText, bgOpacity, textColor, qrColor, qrBgColor, qrScale]);
  ...
  const renderTent = (table: DiningTable) => {
  ...
  const qrUrl = selectedTable ? publicUrl(selectedTable) : window.location.origin;
